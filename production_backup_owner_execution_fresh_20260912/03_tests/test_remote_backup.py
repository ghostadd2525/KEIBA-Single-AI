...
def main() -> int:
    ...
    expect("PACK=production_backup_owner_execution_fresh_20260912" in out, "success_pack_fresh", failures)
    expect("FRESH_LIVE_BACKUP_ONLY=YES" in out, "success_fresh_live_only", failures)
    expect("HISTORICAL_BACKUP_NOT_TOUCHED=YES" in out, "success_historical_untouched", failures)
    expect("AUTO_BACKUP_RESTORE=NO" in out, "success_no_auto_restore", failures)
    expect("FRESH_BACKUP_CANON_PINNED_IN_APPLY_PACK=NO" in out, "success_canon_not_pinned", failures)
    expect("PRODUCTION_022_APPLY_EXECUTION_ALLOWED=NO" in out, "success_apply_exec_no", failures)
    expect("APPLY_EXECUTED=NO" in out, "success_apply_executed_no", failures)
    ...
    remote_text = (PS / "owner_backup_remote.py").read_text(encoding="utf-8")
    expect("migrate(" not in remote_text, "remote_no_migrate", failures)
    expect("restore_rehearse=False" in remote_text, "remote_restore_rehearse_false", failures)
    expect("HISTORICAL_BACKUP_DEST_REFUSED" in remote_text, "remote_historical_dest_token", failures)

    hist_root = tmp / "nested" / remote.HISTORICAL_BACKUP_STAMP
    hist = dict(ok)
    hist["OWNER_BACKUP_TEST_DEST_ROOT"] = str(hist_root)
    hist_before = list(hist_root.rglob("expect_ai.db")) if hist_root.exists() else []
    rc, out = capture_main(hist)
    expect(rc == 2, "historical_dest_halts", failures)
    expect("HALT_REASON=HISTORICAL_BACKUP_DEST_REFUSED" in out, "halt_historical_dest", failures)
    expect("HISTORICAL_BACKUP_NOT_TOUCHED=YES" in out, "historical_dest_untouched_flag", failures)
    expect("AUTO_BACKUP_RESTORE=NO" in out, "historical_dest_no_restore", failures)
    hist_after = list(hist_root.rglob("expect_ai.db")) if hist_root.exists() else []
    expect(hist_after == hist_before, "historical_dest_no_backup_file", failures)
