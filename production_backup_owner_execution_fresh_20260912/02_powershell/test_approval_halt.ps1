# Run OWNER_BACKUP.ps1 without approval. Must not start SSH. Must write output.
$ErrorActionPreference = 'Stop'
$Here = Split-Path -Parent $MyInvocation.MyCommand.Path
$OwnerPs1 = Join-Path $Here 'OWNER_BACKUP.ps1'
$td = Join-Path ([System.IO.Path]::GetTempPath()) ('bakv2halt-' + [guid]::NewGuid().ToString('N'))
[void][System.IO.Directory]::CreateDirectory($td)
$env:USERPROFILE = $td
$env:OWNER_PRODUCTION_BACKUP_APPROVED = $null
Remove-Item Env:OWNER_PRODUCTION_BACKUP_APPROVED -ErrorAction SilentlyContinue
$env:EXPECT_AI_ALLOW_MIGRATION_022 = $null
...
$logs = @(Get-ChildItem -LiteralPath (Join-Path $td 'Downloads') -Filter 'production_backup_owner_execution_fresh_20260912_output_*.txt' -ErrorAction SilentlyContinue)
$fail = 0
...
Expect ([string]$out -match 'OWNER_PRODUCTION_BACKUP_APPROVED_UNSET') 'halt_reason_unset'
