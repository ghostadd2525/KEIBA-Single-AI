# PowerShell 5.1. Owner-only fresh Production backup (live DB).
# Owner runs this file only. Cursor does not run this on Production.
# Sends pregenerated owner_backup_stdin.py byte-for-byte on SSH stdin.
# Does not concatenate Python files at runtime. No SCP. No remote mkdir.
# No sudo. No persistent env change. No 022 APPLY.
# Wrapper never sets OWNER_PRODUCTION_BACKUP_APPROVED.
# v3 ZIP SHA256=6a36403eb758fb2a176480bd8fb05fc001a1d878888513e49588ca18cc6327f8
# failed on future-import syntax after concatenation. Do not overwrite or rerun v3.
$ErrorActionPreference = 'Continue'
$Key = 'C:\Users\Mr.me\Downloads\expect-beta-tokyo.pem'
$Target = 'ubuntu@13.231.5.5'
$Here = Split-Path -Parent $MyInvocation.MyCommand.Path
$StdinPy = Join-Path $Here 'owner_backup_stdin.py'
$Downloads = Join-Path $env:USERPROFILE 'Downloads'
$Stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$OutLog = Join-Path $Downloads ('production_backup_owner_execution_fresh_20260912_output_' + $Stamp + '.txt')
$TimeoutMs = 180000
...
Set-Content -LiteralPath $OutLog -Value 'PRODUCTION_BACKUP_OWNER_EXECUTION_FRESH_20260912' -Encoding UTF8
Write-Log ('STARTED=' + (Get-Date -Format 'yyyy-MM-ddTHH:mm:ss'))
Write-Log 'WRAPPER_VERSION=owner_backup_fresh_20260912'
Write-Log 'FRESH_LIVE_BACKUP_ONLY=YES'
Write-Log 'V5_APPLY_ZIP_OVERWRITE=NO'
Write-Log 'V5_OWNER_EXECUTION_ALLOWED=NO'
Write-Log 'HISTORICAL_BACKUP_NOT_APPLY_CANON=YES'
Write-Log 'HISTORICAL_BACKUP_DELETE=NO'
Write-Log 'HISTORICAL_BACKUP_OVERWRITE=NO'
Write-Log 'AUTO_BACKUP_RESTORE=NO'
Write-Log 'V4_BACKUP_ZIP_OVERWRITE=NO'
Write-Log 'V3_PACK_RUNTIME_PAYLOAD_SYNTAX=FAILED'
Write-Log 'V3_ZIP_OVERWRITE=NO'
Write-Log 'V3_RERUN=NO'
...
$applyApproved = [string]$env:OWNER_PRODUCTION_022_APPLY_APPROVED
if ($applyApproved -eq '1') {
    Write-Log 'SSH_STARTED=NO'
    Write-Log 'HALT_REASON=APPLY_APPROVAL_NOT_ALLOWED_IN_BACKUP_PACK'
    Write-Log 'OWNER_BACKUP_STATUS=FAIL'
    Write-Log 'PRODUCTION_022_APPLY_EXECUTION_ALLOWED=NO'
    Write-Output 'OWNER_BACKUP_STATUS=FAIL'
    Write-Output 'HALT_REASON=APPLY_APPROVAL_NOT_ALLOWED_IN_BACKUP_PACK'
...
Write-Log 'STDIN_SOURCE=owner_backup_stdin.py'
