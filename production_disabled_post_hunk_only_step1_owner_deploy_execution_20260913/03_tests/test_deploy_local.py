...
def base_env(tree: Path, db: Path, backup: Path, restart_log: Path) -> dict[str, str]:
    env = {
        "OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED": "1",
...
def main() -> int:
    failures: list[str] = []
    expect(len(deploy.DEPLOY_TARGETS) == 11, "eleven_targets", failures)
    ...
    expect(deploy.RESTART_ARGV == ("systemctl", "restart", "expect-ai.service"), "restart_argv", failures)

    rc, out = capture_main({})
    expect(rc == 2, "refuse_unapproved", failures)
    expect("HALT_REASON=OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED_UNSET" in out, "halt_unapproved", failures)
    ...
    expect(lines == ["systemctl restart expect-ai.service"], "happy_restart_log_single", failures)
    ...
    expect(restart_lines(log8) == ["systemctl restart expect-ai.service", "systemctl restart expect-ai.service"], "after_restart_two_restarts", failures)
