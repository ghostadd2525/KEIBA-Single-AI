#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Single SSH stdin payload for 工程1 hunk-only code deploy. Pregenerated. Do not concatenate."""
...
APPROVAL = "OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED"
RESTART_UNIT = "expect-ai.service"
RESTART_ARGV = ("systemctl", "restart", RESTART_UNIT)
...
def refuse_unapproved() -> None:
    if (os.environ.get(APPROVAL) or "").strip() != "1":
        raise Halt("OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED_UNSET")
...
def collect_production_env(*, after: bool = False) -> dict[str, object]:
    ...
    if test_mode() and (os.environ.get("OWNER_DEPLOY_TEST_ENV") or "").strip() == "1":
        ...
        rc, show = run_cmd(["systemctl", "show", RESTART_UNIT, "--property=Id,LoadState,MainPID,Environment,EnvironmentFiles,ActiveState,FragmentPath"])
...
def maybe_recovery_restart() -> None:
    if STATE.restart_executed and STATE.restored:
...
def run() -> int:
    STATE.deadline = Deadline(remote_hard_deadline_s())
    emit("PACK", PACK)
    emit("KIND", "OWNER_STEP1_CODE_DEPLOY_EXECUTION")
