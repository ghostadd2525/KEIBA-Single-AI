#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Single SSH stdin payload for 022 schema APPLY v5. Pregenerated. Do not concatenate."""
...
# This v5 design pack does not pin an APPLY backup canon.
APPLY_BACKUP_CANON = "PENDING_FRESH_PRODUCTION_BACKUP"
...
def is_historical_backup_path(path: Path) -> bool:
    text = path.as_posix()
    return text == HISTORICAL_BACKUP_PATH or "20260911T175303Z" in text
...
def backup_path() -> Path:
    if test_mode():
        ...
        if override:
            path = Path(override)
            if is_historical_backup_path(path):
