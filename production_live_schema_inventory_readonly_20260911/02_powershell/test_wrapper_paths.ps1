...
# Extracts functions from OWNER_READONLY.ps1 via AST after PARSE_ERROR_COUNT=0.
$ErrorActionPreference = 'Stop'
$Here = Split-Path -Parent $MyInvocation.MyCommand.Path
$OwnerPs1 = Join-Path $Here 'OWNER_READONLY.ps1'
$fail = 0
...
$needed = @('Stop-ProcessTree', 'Get-TaskText', 'Get-WrapperAuditStatus', 'Invoke-TimedProcess')
...
$td = Join-Path ([System.IO.Path]::GetTempPath()) ('dryrunv2paths-' + [guid]::NewGuid().ToString('N'))
...
$sf = Invoke-TimedProcess -FileName '/no/such/dryrun_v2_ssh_binary' -Arguments '' -TimeoutMs 5000 -StdinText $null
