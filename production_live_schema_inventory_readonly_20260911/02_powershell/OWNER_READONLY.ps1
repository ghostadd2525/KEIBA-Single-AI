# PowerShell 5.1. Owner-only. READ-ONLY live schema inventory. No HTTP. No writes.
# No GET. No POST. sqlite_master / PRAGMA only. No row bodies.
# Cursor does not run this. Do not overwrite v1-v4 review ZIP. No Production writes.
# No SCP. No GitHub. No 019 apply. No POST.
# Remote Python must not call HTTP.
...
$AuditPy = Join-Path $Here 'live_schema_inventory.py'
$Downloads = Join-Path $env:USERPROFILE 'Downloads'
$Stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$OutLog = Join-Path $Downloads ('production_live_schema_inventory_readonly_20260911_output_' + $Stamp + '.txt')
$TimeoutMs = 180000
...
Set-Content -LiteralPath $OutLog -Value 'PRODUCTION_LIVE_SCHEMA_INVENTORY_READONLY_20260911' -Encoding UTF8
Write-Log ('STARTED=' + (Get-Date -Format 'yyyy-MM-ddTHH:mm:ss'))
Write-Log 'WRAPPER_VERSION=owner_readonly_live_schema_inventory_20260911'
Write-Log 'HTTP_CALLED=NO'
Write-Log 'HEALTH_GET_CALLED=NO'
...
Write-Log 'REMOTE_LOCALHOST_GET_ONLY=NO'
