/**
 * POST /api/ops/approvals/:id/reject — Version8.8
 * body: { reason } → Knowledge Base (rejected_patterns)
 */
...
export async function onRequestPost(context) {
  ...
  if (!reason) return jsonError("BAD_REQUEST", "reject reason required", 400);
  ...
  const proxied = await aiFetch(
    ...
    `/v1/admin/approvals/${encodeURIComponent(id)}/reject`,
    {
      method: "POST",
      headers,
      body: JSON.stringify({ actor: session.id || "admin", reason }),
      timeoutMs: 30000,
    }
  );
  ...
  return jsonError(
    ...
    (proxied && proxied.error && proxied.error.message) || "reject failed",
    502
  );
}
