...
# Wrapper never sets OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED.
...
$OutLog = Join-Path $Downloads ('production_disabled_post_hunk_only_step1_owner_deploy_execution_v2_20260913_output_' + $Stamp + '.txt')
$TimeoutMs = 400000
...
function Test-OwnerApprovalReady {
    ...
    if ($Approved -ne '1') {
        return @{
            Ready = $false
            Reason = 'OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED_UNSET'
        }
    }
    return @{
        Ready = $true
...
Set-Content -LiteralPath $OutLog -Value 'PRODUCTION_DISABLED_POST_HUNK_ONLY_STEP1_OWNER_DEPLOY_EXECUTION_V2_20260913' -Encoding UTF8
Write-Log ('STARTED=' + (Get-Date -Format 'yyyy-MM-ddTHH:mm:ss'))
Write-Log 'WRAPPER_VERSION=owner_deploy_step1_v2_20260913'
...
$approvedText = [string]$env:OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED
if ($v1Approved -eq '1' -and $approvedText -ne '1') {
    Write-Log 'SSH_STARTED=NO'
...
$remoteCmd = 'OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED=1 PYTHONDONTWRITEBYTECODE=1 python3 -'
