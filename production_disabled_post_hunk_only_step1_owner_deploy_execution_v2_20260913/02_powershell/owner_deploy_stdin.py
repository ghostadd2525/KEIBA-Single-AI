#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Single SSH stdin payload for 工程1 hunk-only code deploy v2. Pregenerated. Do not concatenate. sudo -n restart only."""
...
APPROVAL = "OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED"
...
def refuse_unapproved() -> None:
    ...
    v2 = (os.environ.get(APPROVAL) or "").strip()
    ...
    if v2 != "1":
        raise Halt("OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED_UNSET")
...
def run() -> int:
    STATE.deadline = Deadline(remote_hard_deadline_s())
    emit("PACK", PACK)
    emit("KIND", "OWNER_STEP1_CODE_DEPLOY_EXECUTION_V2")
    emit("PACK_VERSION", "v2")
    emit("ROOT_CAUSE_ADDRESSED", "SUDO_REQUIRED_FOR_SYSTEMCTL_RESTART")
