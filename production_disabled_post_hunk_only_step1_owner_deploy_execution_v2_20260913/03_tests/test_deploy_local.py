...
def base_env(tree: Path, db: Path, backup: Path, restart_log: Path) -> dict[str, str]:
    env = {
        "OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED": "1",
...
def main() -> int:
    ...
    expect("HALT_REASON=OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED_UNSET" in out, "halt_unapproved", failures)
    ...
    v1only.pop("OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED", None)
