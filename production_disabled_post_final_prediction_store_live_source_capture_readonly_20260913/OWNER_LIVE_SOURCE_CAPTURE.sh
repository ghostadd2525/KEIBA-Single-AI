...
echo "===== BEGIN OWNER_READ_ONLY_FINAL_PREDICTION_STORE_CAPTURE ====="
echo "PACK=production_disabled_post_final_prediction_store_live_source_capture_readonly_20260913"
...
def emit_judgment(captured: int) -> str:
    ...
    out("NEXT_STEP=INDEPENDENT_REVIEW_OF_READONLY_FINAL_PREDICTION_STORE_CAPTURE")
    return result


def main() -> int:
    reset_state()
    out("PACK=production_disabled_post_final_prediction_store_live_source_capture_readonly_20260913")
    out("KIND=OWNER_READ_ONLY_FINAL_PREDICTION_STORE_CAPTURE")
...
rc=$?
echo "===== END OWNER_READ_ONLY_FINAL_PREDICTION_STORE_CAPTURE ====="
exit "$rc"
