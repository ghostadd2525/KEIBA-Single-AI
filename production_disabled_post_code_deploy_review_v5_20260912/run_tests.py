...
Pass condition is a live unittest run after:
  ...
  3. overlaying files/
  4. asserting _connect_idempotent() does not set journal_mode
  5. executing the 51-test suite (must PASS)
  6. executing test_separate_process_concurrency 20 times (all PASS)
  7. executing the 51-test suite again (must PASS)
  8. asserting Ran 51 tests and OK on each full-suite run
...
def verify_wal_init_contract(tree: Path) -> None:
    repo = tree / REPO_REL
    db_path = tree / DB_REL
    if not repo.is_file() or not db_path.is_file():
        raise RuntimeError("wal_contract_files_missing")
    ...
    ensure_src = _function_source(db_path, "ensure_wal_journal")
    migrate_src = _function_source(db_path, "migrate")
    if "PRAGMA journal_mode = WAL" not in ensure_src:
        raise RuntimeError("ensure_wal_journal_missing_wal_write")
    if 'current == "wal"' not in ensure_src and "current == 'wal'" not in ensure_src:
        raise RuntimeError("ensure_wal_journal_missing_read_first")
    if "ensure_wal_journal(conn)" not in migrate_src:
        raise RuntimeError("migrate_missing_ensure_wal_journal")
    if "except" in ensure_src and "OperationalError" in ensure_src:
        raise RuntimeError("ensure_wal_journal_swallows_lock")
    if "journal_mode" in db_text and "except sqlite3.OperationalError" in ensure_src:
        raise RuntimeError("ensure_wal_journal_swallows_lock")
...
def main() -> int:
    ...
    tmp = tempfile.mkdtemp(prefix="disabled-post-v5-overlay-")
    try:
        tree = Path(tmp) / "tree"
        ...
        verify_wal_init_contract(tree)
        _print("CONNECT_IDEMPOTENT_SETS_JOURNAL_MODE=NO")
        _print("WAL_INIT_SITE=app.data.db.ensure_wal_journal via migrate()")
        env = os.environ.copy()
