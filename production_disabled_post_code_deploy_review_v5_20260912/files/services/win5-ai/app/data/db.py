...
def ensure_wal_journal(conn: sqlite3.Connection) -> str:
    """Single WAL init site. Do not call from per-connection helpers.

    Changing ``journal_mode`` rewrites the journal file. Concurrent
    processes that each issue ``PRAGMA journal_mode = WAL`` on connect
    can raise ``sqlite3.OperationalError: database is locked``. Read the
    current mode first; only write when it is not already WAL. Uses
    autocommit because SQLite rejects journal_mode changes inside a
    transaction. Does not swallow database-is-locked.
    """
    ...
    try:
        ...
        current = str(row[0] if row is not None else "").strip().lower()
        if current == "wal":
            return current
        after = conn.execute("PRAGMA journal_mode = WAL").fetchone()
        return str(after[0] if after is not None else "").strip().lower()
    finally:
...
def migrate(conn: sqlite3.Connection | None = None) -> list[str]:
    ...
    try:
        ...
        conn.commit()
        # WAL is initialized here once per migrate(), not on every
        # save_idempotent() connection. Later migrate() calls only read
        # journal_mode when it is already WAL.
