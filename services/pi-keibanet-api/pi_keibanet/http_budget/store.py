...
def component_counts(
    ...
    *,
    ...
    expr = "COUNT(*)" if count_all else "COALESCE(SUM(consumed), 0)"
    rows = conn.execute(
        f"""
        SELECT component, {expr} AS used
        FROM reservations
        WHERE window_id = ?
        GROUP BY component
        """,
...
def usage_snapshot(
    ...
    *,
    ...
    by_component = component_counts(conn, window_id, count_all=count_all)
    expr = "COUNT(*)" if count_all else "COALESCE(SUM(consumed), 0)"
    by_host_rows = conn.execute(
        f"""
        SELECT host, {expr} AS used
        FROM reservations
        WHERE window_id = ?
        GROUP BY host
        """,
...
def rolling_cutoff_iso(now: datetime | None, window_sec: int) -> str:
    current = now or datetime.now(timezone.utc)
    if current.tzinfo is None:
        current = current.replace(tzinfo=timezone.utc)
    else:
        current = current.astimezone(timezone.utc)
    return (current - timedelta(seconds=max(0, window_sec))).strftime("%Y-%m-%dT%H:%M:%SZ")
...
def rolling_usage(
    ...
    *,
    ...
    """Count consumed reservations in a rolling wall-clock window (UTC ISO)."""
    if window_sec <= 0:
        return {"total": 0, "by_component": {}, "by_host": {}, "cutoff": ""}
    cutoff = rolling_cutoff_iso(now, window_sec)
    consume_filter = "" if count_all else "AND consumed = 1"
    expr = "COUNT(*)" if count_all else "COALESCE(SUM(consumed), 0)"
    by_component_rows = conn.execute(
        f"""
        SELECT component, {expr} AS used
        FROM reservations
        WHERE reserved_at >= ? {consume_filter}
        GROUP BY component
        """,
        (cutoff,),
    ).fetchall()
    ...
    by_component = {str(r["component"]): int(r["used"]) for r in by_component_rows}
    return {
        "cutoff": cutoff,
        "total": sum(by_component.values()),
        "by_component": by_component,
