...
class MaidenW3W5StagingTests(unittest.TestCase):
    ...
    def test_w3a_degraded_stops_new_supply_without_queue_write(self) -> None:
        ...
        self.assertEqual(report.supply_skipped_reason, "c4_not_healthy:DEGRADED")
    ...
    def test_w3c_runner_degraded_still_runs_w3c(self) -> None:
        ...
        handoff = HandoffReport(
            supply_skipped_reason="c4_not_healthy:DEGRADED",
    ...
    def test_w5_migrate_default_dry_run_then_apply(self) -> None:
        ...
        after_first = hashlib.sha256(cfg.queue_path.read_bytes()).hexdigest()
        second = migrate.migrate(cfg.queue_path, apply=True)
        self.assertEqual(second["planned_count"], 0)
        self.assertEqual(second["applied_count"], 0)
        self.assertEqual(hashlib.sha256(cfg.queue_path.read_bytes()).hexdigest(), after_first)
    ...
    def test_historical_unknown_days_are_not_zero_maidens(self) -> None:
        ...
        self.assertEqual(report["existing_w3_count"]["2024"], EXISTING_W3)
        self.assertEqual(report["existing_w3_count"]["2025"], 0)
        self.assertEqual(report["existing_w3_count"]["2026"], 0)
        existing_2024 = set(report["existing_w3_race_ids"]["2024"])
        self.assertEqual(len(existing_2024), EXISTING_W3)
        self.assertTrue(existing_2024.isdisjoint(report["missing_maiden_race_ids"]["2024"]))
        self.assertTrue(existing_2024.isdisjoint(report["planned_new_race_ids"]))
    ...
    def test_w5_attempt_count_single_increment_on_exception_and_http(self) -> None:
        ...
        http500 = run_one("2021000002", NetkeibaFetchError("HTTP 500"))
        ...
        http403 = run_one("2021000003", NetkeibaFetchError("HTTP 403"))
    ...
    def test_c4_healthy_only_allows_new_supply(self) -> None:
        cfg = _w3_cfg(self.tmp, w3a_handoff_dry_run=True, w3a_max_enqueue_per_run=2)
        _seed_w3_world(cfg, self.tmp)
        _write_c4_health(self.tmp, "HEALTHY")
        report = run_w3a_handoff(cfg)
        self.assertIsNone(report.supply_skipped_reason)
        self.assertEqual(report.planned_new_race_ids, ["202506010101", "202506010102"])
        self.assertEqual(report.http_request_count, 0)

    def test_c4_non_healthy_states_stop_supply(self) -> None:
        for state in ("DEGRADED", "BLOCKED", "COOLDOWN", "UNAVAILABLE", "WEIRD"):
            with self.subTest(state=state):
                cfg = _w3_cfg(self.tmp, w3a_handoff_dry_run=False, w3a_max_enqueue_per_run=2)
                _seed_w3_world(cfg, self.tmp)
                _write_c4_health(self.tmp, state)
                before = cfg.queue_path.read_bytes()
                report = run_w3a_handoff(cfg)
                self.assertEqual(report.c4_source_health_state, state)
                self.assertEqual(report.supply_skipped_reason, f"c4_not_healthy:{state}")
                self.assertEqual(report.planned_new_race_ids, [])
                self.assertFalse(report.queue_written)
                self.assertEqual(report.http_request_count, 0)
                self.assertEqual(cfg.queue_path.read_bytes(), before)

    def test_w5_http_retry_allowlist(self) -> None:
        now = datetime(2026, 9, 7, 12, 0, tzinfo=timezone.utc)
        past = "2026-09-07T11:00:00Z"
        retryable = {429, 500, 503}
        permanent = {400, 401, 403, 404}
        for status in sorted(retryable | permanent):
            row = {
                "queue_status": "fetch_failed",
            ...
            got = _is_retryable_fetch_failed(row, now=now, max_attempts=3)
            if status in retryable:
                self.assertTrue(got, status)
            else:
                self.assertFalse(got, status)

    def test_w5_migrate_attempt_cap_excluded(self) -> None:
        migrate = _load_script("w5_fetch_failed_retry_migrate.py")
        cfg = _w5_cfg(self.tmp)
        cfg.w5_root.mkdir(parents=True, exist_ok=True)
        rows = {}
        for hid in FAILED_W5:
            ...
            rows[hid] = rec
        w5_save_queue(cfg.queue_path, rows)
        before = hashlib.sha256(cfg.queue_path.read_bytes()).hexdigest()
        report = migrate.migrate(cfg.queue_path, apply=True)
        self.assertEqual(report["planned_count"], 0)
        self.assertEqual(report["applied_count"], 0)
        self.assertEqual(hashlib.sha256(cfg.queue_path.read_bytes()).hexdigest(), before)
        self.assertTrue(all(s["reason"] == "attempt_cap" for s in report["skipped"]))

    def test_historical_known_existing_missing_sets(self) -> None:
        hist = _load_script("w3_maiden_historical_dry_run.py")
        cfg = _w3_cfg(self.tmp, w3a_max_enqueue_per_run=2)
        _seed_w3_world(cfg, self.tmp)
        report = hist.run_historical_dry_run(cfg)
        known = set(report["known_maiden_race_ids"]["2024"]) | set(report["known_maiden_race_ids"]["2025"]) | set(report["known_maiden_race_ids"]["2026"])
        existing = set(report["existing_w3_race_ids"]["2024"]) | set(report["existing_w3_race_ids"]["2025"]) | set(report["existing_w3_race_ids"]["2026"])
        missing = set(report["missing_maiden_race_ids"]["2024"]) | set(report["missing_maiden_race_ids"]["2025"]) | set(report["missing_maiden_race_ids"]["2026"])
        self.assertEqual(missing, known - existing)
        self.assertEqual(len(existing), EXISTING_W3)
        self.assertTrue(existing.isdisjoint(missing))
        self.assertTrue(existing.isdisjoint(set(report["planned_new_race_ids"])))
        self.assertEqual(report["planned_new_race_ids"], ["202506010101", "202506010102"])
        self.assertNotIn("202606010101", report["planned_new_race_ids"])

    def test_pr_runtime_files_contain_p0_contracts(self) -> None:
        root = ROOT
        required = [
            root / "pi_keibanet/w3_maiden/config.py",
            root / "pi_keibanet/w3_maiden/handoff.py",
            root / "scripts/w3_maiden_page_c_acquire_run.py",
            root / "pi_keibanet/w5_maiden_history/config.py",
            root / "pi_keibanet/w5_maiden_history/acquisition.py",
            root / "scripts/w5_maiden_history_acquire_run.py",
            root / "scripts/w5_fetch_failed_retry_migrate.py",
            root / "scripts/w3_maiden_historical_dry_run.py",
            root / "tests/test_maiden_w3w5_staging_patch.py",
        ]
        for path in required:
            self.assertTrue(path.is_file(), path)
        handoff = (root / "pi_keibanet/w3_maiden/handoff.py").read_text(encoding="utf-8")
        acq = (root / "pi_keibanet/w5_maiden_history/acquisition.py").read_text(encoding="utf-8")
        self.assertIn('!= "HEALTHY"', handoff)
        self.assertNotIn("status not in BLOCK_HTTP_STATUSES", acq)
        self.assertIn("_is_retryable_http_status", acq)
        self.assertIn("500 <= status <= 599", acq)
