...
class TargetPredictionFallbackTest(unittest.TestCase):
    def test_target_reaches_resolver_success_not_race_not_found(self) -> None:
        with isolated_env(engine="real"):
            os.environ["CATALOG_INDEX_DISABLE_PI"] = "1"
            from app.data.catalog_index import reset_catalog_index_for_tests
            from app.data.race_resolver import reset_resolver_for_tests
            from app.engine.adapters.prediction_adapter import RealAiPredictionSource

            reset_catalog_index_for_tests()
            reset_resolver_for_tests()

            src = RealAiPredictionSource()
            bundle, meta = src.get_with_meta(TARGET_CATALOG_ID)
            self.assertIsNotNone(meta)
            assert meta is not None
            self.assertEqual(meta.get("canonical_race_id"), TARGET_CORE_ID)
            self.assertEqual(meta.get("source_race_id"), TARGET_CATALOG_ID)
            self.assertEqual(meta.get("core_race_id"), TARGET_CORE_ID)
            # Production AFTER (CSV tree present, this race absent) = market_feature_missing.
            # This sandbox has no AI_PLATFORM_ROOT → platform_missing, which is still
            # past race_not_found (resolver succeeded).
            self.assertNotEqual(meta.get("fallback_reason"), "race_not_found")
            self.assertNotEqual(meta.get("detail"), "no resolvable core race_id")
            self.assertNotIn("RESTORED_V2", str(meta.get("engine_source") or ""))
