...
def core_race_id_from_catalog(
    *,
    ...
    if stored:
        core = CoreRaceRef.parse(stored)
        if core:
            return core.as_id(), core.jra_venue_code
    core = CoreRaceRef.from_catalog_row(
        date=date,
        course=course,
        race_no=race_no,
...
class CatalogIndex:
    ...
    def add_row(self, payload: dict[str, Any]) -> CatalogRace | None:
        ...
        if not catalog_id or not date or not course or race_no is None:
            return None
        numeric_ref = NumericRaceRef.parse(str(payload.get("numeric_race_id") or ""))
        numeric = numeric_ref.as_id() if numeric_ref else re.sub(r"\D", "", str(payload.get("numeric_race_id") or ""))
        ...
        row = CatalogRace(
            ...
            label_no=label,
            feature_lookup_key=FeatureLookupRef.from_catalog(catalog_id).as_id(),
        )
        self._index_row(row)
        return row
