# PowerShell 5.1. Owner-only. READ-ONLY Production prediction-path smoke v2.
# Cursor does not run this. Do not run v1 pack.
# No Production writes. No SCP. No GitHub.
# Remote Python may GET http://127.0.0.1:8000 only. Never POST /v1/prediction-runs.
# Detail GET uses race_id from the same-run list GET only. No DB historical race_id.
$ErrorActionPreference = 'Continue'
$Key = 'C:\Users\Mr.me\Downloads\expect-beta-tokyo.pem'
$Target = 'ubuntu@13.231.5.5'
$Here = Split-Path -Parent $MyInvocation.MyCommand.Path
$AuditPy = Join-Path $Here 'prediction_smoke_readonly.py'
$Downloads = Join-Path $env:USERPROFILE 'Downloads'
$Stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$OutLog = Join-Path $Downloads ('production_prediction_path_smoke_readonly_v2_20260910_output_' + $Stamp + '.txt')
$TimeoutMs = 180000
...
Set-Content -LiteralPath $OutLog -Value 'PRODUCTION_PREDICTION_PATH_SMOKE_READONLY_V2_20260910' -Encoding UTF8
Write-Log ('STARTED=' + (Get-Date -Format 'yyyy-MM-ddTHH:mm:ss'))
Write-Log 'WRAPPER_VERSION=owner_readonly_prediction_smoke_v2'
Write-Log 'V1_DO_NOT_RUN=YES'
Write-Log 'OWNER_PACK_SAFE_RACE_SELECTION=YES'
Write-Log 'HISTORICAL_DB_RACE_LIVE_INFERENCE_ALLOWED=NO'
Write-Log 'DETAIL_GET_ONLY_FROM_CURRENT_LIST=YES'
Write-Log 'V2_STDOUT_DRAIN_CONCURRENT=YES'
Write-Log 'V2_STDERR_DRAIN_CONCURRENT=YES'
...
Write-Output ('OUTPUT=' + $OutLog)
Write-Output 'V1_DO_NOT_RUN=YES'
exit $st.WrapperExit
