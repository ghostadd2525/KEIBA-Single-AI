...
def systemd_fixture(win5: Path, secret: str = DUMMY_SECRET) -> dict[str, str]:
    return {
        ...
        "ExecStart": (
            ...
            "argv[]=/usr/bin/python3 -m app.main --host 127.0.0.1 --token %s ; "
            "ignore_errors=no }" % secret
        ),
        "Environment": "PREDICTION_RUNS_ENABLED=0 OTHER_SECRET=%s" % DUMMY_ENV_SECRET,
        "EnvironmentFiles": "",
        "MainPID": "0",
        "User": "ubuntu",
        "ActiveState": "active",
...
def run_main(win5: Path, extra_present: str | None = None) -> tuple[int, str]:
    if extra_present or not (win5 / "app" / "main.py").is_file():
        copy_v4_tree(win5, extra_present=extra_present)
    fixture = systemd_fixture(win5)
    buf = io.StringIO()
    with patch.object(lc, "systemd_show", return_value=fixture):
...
class IndexContractTests(unittest.TestCase):
    ...
    def test_good_partial_unique_is_ok(self) -> None:
        ...
        self.assertEqual(got["INDEX_TBL_NAME"], "predictions")
        self.assertEqual(got["INDEX_INFO_COLS"], "['idempotency_key']")
    ...
    def test_unique_without_predicate_is_not_ok(self) -> None:
        ...
        self.assertEqual(got["INDEX_SQL_PREDICATE_OK"], "NO")
    ...
    def test_extra_index_column_is_not_ok(self) -> None:
        ...
        self.assertEqual(got["INDEX_CONTRACT"], "MISMATCH")
        self.assertEqual(got["INDEX_INFO_COLS_OK"], "NO")
    ...
    def test_each_must_absent_present_is_stop(self) -> None:
        for rel in lc.MUST_ABSENT:
            with self.subTest(rel=rel):
                with tempfile.TemporaryDirectory() as td:
                    win5 = Path(td) / "services" / "win5-ai"
                    rc, stdout = run_main(win5, extra_present=rel)
...
class ExecStartRedactTests(unittest.TestCase):
    def test_dummy_secret_not_in_stdout(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            win5 = Path(td) / "services" / "win5-ai"
            rc, stdout = run_main(win5)
            ...
            self.assertIn("SYSTEMD_EXECSTART_ARGV_SAFE=", stdout)
            self.assertIn("-m app.main", stdout)
            self.assertIn("--token [REDACTED]", stdout)
            self.assertIn("PREDICTION_RUNS_ENABLED=ZERO", stdout)
            self.assertEqual(rc, 0)

    def test_assignment_form_redacted(self) -> None:
        safe = lc.redact_argv(["/usr/bin/python3", "--password=hunter2", "--port", "8000"])
        self.assertEqual(safe, ["/usr/bin/python3", "--password=[REDACTED]", "--port", "8000"])
        joined = " ".join(safe)
    ...
    def test_owner_scripts_do_not_write_restart_migrate_or_post(self) -> None:
        ...
        for name, text in texts.items():
            for pat in forbidden:
                if pat.startswith(r"sqlite3"):
                    if name == "live_compare.py" or name.endswith(".sh"):
