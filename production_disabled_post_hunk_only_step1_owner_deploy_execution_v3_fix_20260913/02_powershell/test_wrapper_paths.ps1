...
$OwnerPs1 = Join-Path $Here 'OWNER_DEPLOY_V3.ps1'
$fail = 0
...
$needed = @('Stop-ProcessTree', 'Get-TaskText', 'Get-WrapperAuditStatus', 'Get-RemoteDeployState', 'Invoke-TimedProcess', 'Test-OwnerApprovalReady', 'Get-Sha256Hex', 'Test-PackPayloadSha')
...
Expect ($allText.Contains('$env:OWNER_PRODUCTION_STEP1_V3_DEPLOY_APPROVED =') -eq $false) 'wrapper_does_not_set_approval'
Expect ($allText.Contains('$env:OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED =') -eq $false) 'wrapper_does_not_set_v2_approval'
Expect ($allText.Contains('$env:OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED =') -eq $false) 'wrapper_does_not_set_v1_approval'
...
Expect ($allText.Contains('owner_deploy_stdin.py') -eq $true) 'reads_pregenerated_stdin'
...
$td = Join-Path ([System.IO.Path]::GetTempPath()) ('step1paths-' + [guid]::NewGuid().ToString('N'))
...
Set-Content -LiteralPath $committedToPy -Value "import sys,time`nsys.stdout.write('DEPLOY_EXECUTED=YES\nDEPLOY_PHASE=VERIFIED\nRESTART_EXECUTED=YES\n')`nsys.stdout.flush()`ntime.sleep(20)`n" -Encoding ASCII
...
$unknownTo = Get-RemoteDeployState -StdOut ([string]$to.StdOut)
Expect ($unknownTo.DeployExecuted -eq 'UNKNOWN') 'path_timeout_before_commit_executed_unknown'
Expect ($unknownTo.DeployPhase -eq 'UNKNOWN') 'path_timeout_before_commit_phase_unknown'
...
Expect ([string]$cto.StdOut -match 'DEPLOY_EXECUTED=YES') 'path_timeout_after_commit_executed_yes'
Expect ([string]$cto.StdOut -match 'DEPLOY_PHASE=VERIFIED') 'path_timeout_after_commit_phase_committed'
$knownTo = Get-RemoteDeployState -StdOut ([string]$cto.StdOut)
Expect ($knownTo.DeployExecuted -eq 'YES') 'path_timeout_after_commit_state_executed'
Expect ($knownTo.DeployPhase -eq 'VERIFIED') 'path_timeout_after_commit_state_phase'
...
Expect (-not (($knownTo.DeployExecuted -eq 'UNKNOWN') -or ($knownTo.DeployPhase -eq 'UNKNOWN'))) 'path_timeout_after_commit_not_unknown'
...
Expect ($g1.Reason -eq 'OWNER_PRODUCTION_STEP1_V3_DEPLOY_APPROVED_UNSET') 'approval_unset_reason'
