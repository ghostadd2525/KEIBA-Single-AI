...
def choose_engine():
    ...
    if pwsh:
        return pwsh, "pwsh"
    bundled = Path("/tmp/pwsh/pwsh")
    if bundled.is_file() and os.access(str(bundled), os.X_OK):
        return str(bundled), "pwsh"
    return None, None


def resolve_parser_engine():
    if os.environ.get("OWNER_DEPLOY_TEST_NO_PARSER") == "1":
        return None, None
...
def finalize_evidence_and_sums():
    ...
    dest = Path(tempfile.mkdtemp(prefix="step1-sums-")) / "production_disabled_post_hunk_only_step1_owner_deploy_execution_v3_fix_20260913"
...
def main():
    ...
    env.pop("OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED", None)
    env.pop("OWNER_PRODUCTION_STEP1_V3_DEPLOY_APPROVED", None)
    ...
    r = subprocess.run([sys.executable, str(ROOT / "03_tests" / "test_deploy_local.py")], capture_output=True, text=True, env=env)
    print(r.stdout)
    if r.stderr:
        print(r.stderr)
    expect(r.returncode == 0 and "ALL_PASS" in (r.stdout or ""), "deploy_local")
    ...
    tmp = Path(tempfile.mkdtemp(prefix="step1-halt-"))
    downloads = tmp / "Downloads"
    downloads.mkdir()
    # Wrapper SHA gate runs before approval. Sums must match current pack files.
    import generate_sha256sums
    generate_sha256sums.write_sha256sums()
    halt_env = env.copy()
    halt_env["USERPROFILE"] = str(tmp)
    halt_env.pop("OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED", None)
    halt_env.pop("OWNER_PRODUCTION_STEP1_V2_DEPLOY_APPROVED", None)
    halt_env.pop("OWNER_PRODUCTION_STEP1_V3_DEPLOY_APPROVED", None)
    ...
    expect("OWNER_PRODUCTION_STEP1_V3_DEPLOY_APPROVED_UNSET" in (halt.stdout or ""), "ps1_unapproved_reason")
    logs = list(downloads.glob("production_disabled_post_hunk_only_step1_owner_deploy_execution_v3_fix_20260913_output_*.txt"))
    ...
    if logs:
        ...
        expect("PACK_SHA_GATE=PASS" in body, "ps1_log_sha_pass_before_approval")
