# ADR Index — Conversation Platform + Operations Baseline

| ID | Title | Status |
|----|-------|--------|
| [ADR-001](./ADR-001-security-guard-contract.md) | Security Guard Contract | Accepted · Freeze |
| [ADR-002](./ADR-002-tool-manager-contract.md) | Tool Manager Contract | Accepted · Freeze |
| [ADR-003](./ADR-003-prediction-read-only-contract.md) | Prediction Read-Only Contract | Accepted · Freeze |
| [ADR-004](./ADR-004-review-context-contract.md) | ReviewContext Contract | Accepted · Freeze |
| [ADR-005](./ADR-005-conversation-layer-contract.md) | Conversation Layer Contract | Accepted · Freeze |
| [ADR-006](./ADR-006-memory-layer-contract.md) | Memory Layer Contract（V6 Phase 2） | Accepted |
| [ADR-007](./ADR-007-abilityscores-overlay.md) | AbilityScores CE Overlay Passthrough（Hard Lock Exception） | Accepted · Version8.5.1 |
| [ADR-008](./ADR-008-decision-layer.md) | Decision Layer（World 専属） | Accepted · Frozen · M1 Shadow (V91) · Prod not authorized |

**Freeze Report:** [`../releases/v4-conversation-platform-freeze.md`](../releases/v4-conversation-platform-freeze.md)  
**Migration Notes:** [`../releases/v4-conversation-migration-notes.md`](../releases/v4-conversation-migration-notes.md)  
**V6 Memory Report:** [`../releases/v6-phase2-memory-platform-report.md`](../releases/v6-phase2-memory-platform-report.md)  
**Baselines:** [`../baselines/Version8.5.md`](../baselines/Version8.5.md) · [`../baselines/Version8.5.1.md`](../baselines/Version8.5.1.md)

