...
**Parent:** docs/audit/v851-baseline-certification.md · docs/audit/system-regression-audit.md
...
**Production Baseline:** **Version8.5.1**（docs/baselines/Version8.5.1.md）
...
| 対策 | 実装 |
|------|------|
| role 昇格禁止 | makeStubToken は role 非埋込。erifyStubToken は role claim 破棄 |
...
| 対策 | 実装 |
|------|------|
| 共有 | unctions/_lib/adminAuth.js |
| 空 allowlist | admin ではない（
eturn false）→ 403 |
| 判定不能 | USER / 403 |
...
|------|------|
| Session | 
equireAccessSession（JWT/stub ポリシー） |
| Role | 
esolveAuthorization |
...
- PE 2_pool_entry_v2.py / research CE **hash SAME vs HEAD**
...
- Challenge / UI 回帰修正は本フェーズ対象外（先行完了）
...
2. Ops Knowledge/Deploy 週次 JSON 未公開 — No Data（機能追加ではなくデータパイプライン。8.5.1 範囲外）  
