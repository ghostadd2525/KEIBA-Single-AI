...
class BackupContractTests(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = Path(tempfile.mkdtemp(prefix="bak-rehearse-v2-"))
    ...
    def test_flags_remain_unexecuted(self) -> None:
        ...
        self.assertIn("PRODUCTION_BACKUP_DESIGN_APPROVED=YES", flags)
        self.assertIn("PRODUCTION_BACKUP_EXECUTION_ALLOWED=NO", flags)
        self.assertIn("PRODUCTION_APPLY_READY=NO", flags)
        self.assertIn("OWNER_APPLY_APPROVED=NO", flags)
        self.assertIn("LIVE_SCHEMA_022_STRUCTURAL_COMPATIBILITY=YES", flags)
        self.assertIn("PRODUCTION_BACKUP_EXECUTION_PACK_CREATED=YES", flags)
        self.assertIn("OWNER_PRODUCTION_BACKUP_APPROVED=NO", flags)
    ...
    def test_permission_mismatch_during_contract_disposes(self) -> None:
        ...
        def selective(path, mode):
            p = Path(path)
            if mode == 0o600 and p in (self.dest, self.dest.with_name(self.dest.name + ".tmp")):
    ...
    def test_production_backup_executed_yes_only_on_prod_named_source(self) -> None:
        fake_prod = self.tmp / "KEIBA-Single-AI" / "expect_ai.db"
        fake_prod.parent.mkdir(parents=True)
        shutil.copy2(self.src, fake_prod)
        self.assertTrue(bc.is_production_path(fake_prod))
        dest = self.tmp / "prod-named-backup" / "out.db"
        os.environ["OWNER_PRODUCTION_BACKUP_APPROVED"] = "1"
        with mock.patch.object(bc, "emit") as emit:
            code = bc.run_contract(fake_prod, dest)
        self.assertEqual(code, 0)
        values = [c.args[1] for c in emit.call_args_list if c.args[0] == "PRODUCTION_BACKUP_EXECUTED"]
        self.assertEqual(values[-1], "YES")
        self.assertTrue(dest.is_file())
        os.environ.pop("OWNER_PRODUCTION_BACKUP_APPROVED", None)

    def test_failure_leaves_no_completed_dest_or_tmp(self) -> None:
        with mock.patch.object(bc, "read_integrity", return_value="not ok"), mock.patch.object(bc, "emit"):
            code = bc.run_contract(self.src, self.dest)
        self.assertEqual(code, 2)
        self.assertFalse(self.dest.exists())
        self.assertFalse(self.dest.with_name(self.dest.name + ".tmp").exists())
