...
class HunkOnlyReviewTests(unittest.TestCase):
    ...
    def test_readme_and_donot_forbid_execution(self) -> None:
        ...
        for text in (readme, donot):
            ...
            self.assertIn("INDEPENDENT_REVIEW_OF_HUNK_ONLY_CODE_DEPLOY_BUNDLE", text)
    ...
    def test_no_execution_scripts(self) -> None:
        ...
        for path in PACK.rglob("*"):
            ...
            if path.suffix in {".py", ".txt"}:
                if path.name == "test_hunk_only_review.py":
                    continue
    ...
    def test_no_origin_tar_bundled(self) -> None:
        tars = list(PACK.rglob("*.tar.gz")) + list(PACK.rglob("*.tar"))
        self.assertEqual(tars, [])
        flags = parse_flags(PACK / "FLAGS.txt")
        self.assertEqual(flags["BUNDLED_ORIGIN_TAR"], "NO")
