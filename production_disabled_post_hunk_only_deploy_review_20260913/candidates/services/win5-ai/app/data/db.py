...
def inspect_journal_mode(conn: sqlite3.Connection | None = None) -> str:
    """Read-only journal_mode. Does not write WAL or create -wal/-shm.

    Step-1 ``migrate()`` must not change journal_mode. If WAL is needed
    later, that is a separate Owner approval and a step-2 precondition
    before POST enable. This function only inspects; it does not enable
    WAL. OperationalError is not swallowed.
    """
    own = False
    if conn is None:
        conn = connect()
    ...
    try:
        ...
        return str(row[0] if row is not None else "").strip().lower()
    finally:
        if own:
            conn.close()
...
def migrate(conn: sqlite3.Connection | None = None) -> list[str]:
    ...
    try:
        ...
        conn.commit()
        # Step-1: do not change journal_mode. WAL enable is not implicit.
        return applied
    finally:
        if own:
            conn.close()
