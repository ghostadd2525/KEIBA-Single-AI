#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Single SSH stdin payload for 022 schema APPLY v4. Pregenerated. Do not concatenate."""
...
HTTP_MAX_JSON_BYTES = 262144
HTTP_MAX_HTML_BYTES = 524288
COMMIT_BUDGET_S = 15
...
def read_limited(fp: object, max_bytes: int) -> bytes:
...
def http_get(url: str, *, headers: dict[str, str] | None = None, allow_auth_header: bool = False) -> tuple[int, object, str, str]:
    ...
    max_bytes = HTTP_MAX_HTML_BYTES if path in PUBLIC_STATIC_PATHS else HTTP_MAX_JSON_BYTES
    ...
    try:
        with opener.open(req, timeout=http_timeout_s(public)) as resp:
            raw = read_limited(resp, max_bytes)
        ...
        raw = read_limited(exc, max_bytes) if exc.fp else b""
...
def run() -> int:
    ...
    emit("BUSY_TIMEOUT_MS", BUSY_TIMEOUT_MS)
    emit("HTTP_MAX_JSON_BYTES", HTTP_MAX_JSON_BYTES)
    emit("HTTP_MAX_HTML_BYTES", HTTP_MAX_HTML_BYTES)
