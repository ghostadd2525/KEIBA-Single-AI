#!/usr/bin/env python3
"""Hunk-only v2 self-tests + optional live-dependency overlay suite.
...
def run_struct_tests() -> int:
    ...
    suite = loader.discover(str(PACK / "tests"), pattern="test_hunk_only_v2_review.py")
...
def run_overlay() -> int:
    ...
    if not ready:
        ...
        return 3
    tmp = tempfile.mkdtemp(prefix="hunk-only-v2-overlay-")
    try:
        tree = Path(tmp) / "tree"
        extract_base(tree)
...
def main() -> int:
    print("BUNDLE=production_disabled_post_hunk_only_deploy_review_v2_20260913")
    ...
    if rc != 0:
        return rc
    rc = run_overlay()
    print("NEXT_STEP=INDEPENDENT_REVIEW_OF_HUNK_ONLY_V2_BUNDLE")
