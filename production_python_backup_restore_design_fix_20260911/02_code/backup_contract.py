#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""SQLite backup/restore design contract (local rehearsal only).

Production source, when ever opened, must use:
  file:<path>?mode=ro + uri=True + PRAGMA query_only=ON

This pack never creates a Production execution pack and never sets
OWNER_PRODUCTION_BACKUP_APPROVED. Design tests always leave
...
PACK_NAME = "production_python_backup_restore_design_fix_20260911"
...
def run_contract(
    ...
    *,
    ...
    try:
        ...
        try:
            create_dest_exclusive(dest)
            dest_created = True
        except BackupError:
            dest_created = (not dest_existed) and dest.exists()
            raise
        dest_st = path_stat(dest)
        if (dest_st.st_dev, dest_st.st_ino) == (src_before["dev"], src_before["ino"]):
            raise BackupError("DEST_SAME_INODE_AS_SOURCE")
        emit("DEST_PATH", str(dest.resolve()))
        ...
        emit("DEST_SAME_INODE_AS_SOURCE", dest_info["ino"] == src_before["ino"] and dest_info["dev"] == src_before["dev"])

        if not compare_schema(src, dest):
