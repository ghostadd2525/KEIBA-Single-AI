...
def main() -> int:
    ...
    expect("OWNER_SCHEMA_MIGRATIONS_EXACT_MATCH=YES" in out, "owner_migrations_exact", failures)
    expect("SEEDED_MIGRATION_COUNT=22" in out, "owner_migrations_count", failures)
    expect("020_research_corpus_canonical" in out, "owner_020_corpus", failures)
    expect("020_user_challenge_lifecycle" in out, "owner_020_challenge", failures)
    expect("021_user_challenge_point_events" in out, "owner_021_points", failures)
