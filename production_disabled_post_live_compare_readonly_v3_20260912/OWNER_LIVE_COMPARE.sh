...
python3 - <<'PY'
#!/usr/bin/env python3
"""Read-only Production live compare (v3). Stdout only. No writes."""
...
def emit_judgment() -> str:
    ...
    out("NEXT_STEP=INDEPENDENT_REVIEW_OF_OWNER_READ_ONLY_LIVE_COMPARE_V3")
    return compare
...
def main() -> int:
    ...
    if "_error" in show:
        ...
        if load_state.lower() in {"not-found", "error", "bad-setting"}:
