...
def main() -> int:
    ...
    expect(apply.APPLY_BACKUP_CANON == "PINNED_FRESH_PRODUCTION_BACKUP", "backup_canon_pinned", failures)
    expect(apply.APPLY_BACKUP_PATH.endswith("20260912T013332Z/expect_ai.db"), "fresh_backup_path_pinned", failures)
    expect(apply.APPLY_BACKUP_SHA256 == "32dfe70339a7237442d08a032d019ec03abbb8b85d0c90fa39d385e54da07e6a", "fresh_backup_sha_pinned", failures)
    expect(apply.APPLY_BACKUP_IDENTITY == "66305:287343", "fresh_backup_identity_pinned", failures)
    expect(apply.APPLY_BACKUP_SIZE == 107413504, "fresh_backup_size_pinned", failures)
    expect(apply.FRESH_BACKUP_OWNER_LOG_SHA256 == "f3525b6fea85526083224b38a7d48271ae6a356c3af4bf67908efa07c852495d", "owner_log_sha_imported", failures)
    ...
    expect("APPLY_BACKUP_CANON=PINNED_FRESH_PRODUCTION_BACKUP" in out, "unapproved_backup_pinned", failures)
    ...
    base = {
        ...
        "OWNER_APPLY_TEST_BACKUP_SIZE": str(bst.st_size),
    }
    base.update(env_ok())
    ...
    hist_path = dict(base)
    hist_path["OWNER_APPLY_TEST_BACKUP"] = str(tmp / apply.HISTORICAL_BACKUP_STAMP / "expect_ai.db")
    rc, out = capture_main(hist_path)
    expect(rc == 2, "historical_path_halts", failures)
    expect("HALT_REASON=STALE_BACKUP_NOT_APPLY_CANON" in out, "halt_historical_path", failures)
    expect(last_kv(out, "APPLY_EXECUTED") == "NO", "historical_path_not_executed", failures)

    badsize = dict(base)
    badsize["OWNER_APPLY_TEST_BACKUP_SIZE"] = str(int(bst.st_size) + 99)
    rc, out = capture_main(badsize)
    expect(rc == 2, "backup_size_halts", failures)
    expect("HALT_REASON=BACKUP_SIZE_MISMATCH" in out, "halt_backup_size", failures)
    expect(last_kv(out, "APPLY_EXECUTED") == "NO", "backup_size_not_executed", failures)

    badino = dict(base)
    badino["OWNER_APPLY_TEST_BACKUP_INO"] = str(int(bst.st_ino) + 99999)
    rc, out = capture_main(badino)
    expect(rc == 2, "backup_inode_halts", failures)
    expect("HALT_REASON=BACKUP_DRIFT" in out, "halt_backup_inode", failures)
    expect(last_kv(out, "APPLY_EXECUTED") == "NO", "backup_inode_not_executed", failures)
