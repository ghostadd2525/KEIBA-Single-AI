...
V2_CAPTURE_ZIP_SHA = "a1697ba3d020c85dbcc7dba9e40d5c3608623f1181a2722c77419486805a9ea1"
...
class SecretAndEnvTests(unittest.TestCase):
    ...
    def test_systemd_show_does_not_request_environment(self) -> None:
        ...
        self.assertNotIn("/proc/self/environ", src)
        self.assertNotIn("/proc/environ", src)
        self.assertIn("/proc/self/fd/", src)
...
class ReadonlyStaticTests(unittest.TestCase):
    ...
    def test_windows_ps1_does_not_write_production(self) -> None:
        ...
        self.assertIn("OUTDIR_EXISTS=YES", text)
        self.assertIn("SSH_SKIPPED=YES", text)
        self.assertNotIn("New-Item -ItemType Directory -Force", text)
        self.assertIn("New-Item -ItemType Directory -Path $OutDir", text)
...
class PackContractTests(unittest.TestCase):
    def test_flags_and_readme_hold_invariants(self) -> None:
        ...
        for text in (flags, readme, forbid):
            ...
            self.assertIn("INDEPENDENT_REVIEW_OF_READONLY_LIVE_SOURCE_CAPTURE_V3", text)
        self.assertIn(V5_ZIP_SHA, flags)
        self.assertIn(V6_ZIP_SHA, flags)
        self.assertIn(V1_CAPTURE_ZIP_SHA, flags)
        self.assertIn(V2_CAPTURE_ZIP_SHA, flags)
        self.assertIn(OWNER_LOG_SHA, flags)
        self.assertIn("SCP_UPLOAD=NO", flags)
        self.assertIn("GIT_PULL=NO", flags)
        self.assertIn("O_NOFOLLOW=YES", flags)
        self.assertIn("LSTAT_REGULAR_ONLY=YES", flags)
        self.assertIn("FSTAT_REGULAR_RECHECK=YES", flags)
        self.assertIn("PROC_SELF_FD_CONTAINMENT=YES", flags)
        self.assertIn("OPEN_INODE_RECHECK=YES", flags)
        self.assertIn("OUTDIR_EXISTING_REUSE=NO", flags)
...
class ReadOnlyOpenTests(unittest.TestCase):
    def test_read_readonly_uses_o_rdonly_and_nofollow(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            rel = "app/main.py"
            path = root / rel
            ...
            with patch.object(os, "open", side_effect=wrapped):
                data = cap.read_readonly(path, win5_real=root.resolve(), rel=rel)
            self.assertEqual(data, b"abc")
            self.assertTrue(opened)
            expected = os.O_RDONLY | os.O_NOFOLLOW
            self.assertTrue(all(flags == expected for flags in opened))
            self.assertTrue(all(flags & os.O_NOFOLLOW for flags in opened))
