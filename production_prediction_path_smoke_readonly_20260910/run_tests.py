...
def test_ps():
    ...
    expect("prediction-runs" not in ps.lower() or "Never POST" in ps, "ps_mentions_no_post")
    expect("POST_ENDPOINT_CALLED=NO" in ps, "no_post_flag")
    expect("PRODUCTION_CHANGED=NO" in ps, "no_prod_change")
    expect("SYSTEMD_MUTATING=NO" in ps, "no_systemd_mutate")
