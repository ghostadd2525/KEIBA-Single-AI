# PowerShell 5.1. Owner-only. READ-ONLY Production prediction-path smoke.
# Cursor does not run this. No Production writes. No SCP. No GitHub.
...
$OutLog = Join-Path $Downloads ('production_prediction_path_smoke_readonly_20260910_output_' + $Stamp + '.txt')
$TimeoutMs = 180000
...
Set-Content -LiteralPath $OutLog -Value 'PRODUCTION_PREDICTION_PATH_SMOKE_READONLY_20260910' -Encoding UTF8
Write-Log ('STARTED=' + (Get-Date -Format 'yyyy-MM-ddTHH:mm:ss'))
Write-Log 'WRAPPER_VERSION=owner_readonly_prediction_smoke_1'
