...
V6_SNAPSHOT = "2114c953b57461f73d0450384f38df2a2b086a51e53591c77cc70b448cbde5f7"
V6_DB = "81fc91adbe53aa091841fd9ea097b7cbd63cf4ede4a3699bf66e1c7a2343136f"
V6_REPO = "9624699ec48d2af8b1fdc24ac81b8ae436c77ffa92a7f0b1378ecd17248b4c08"
STALE_V4_DB = "caa14ebd448a40164dab427d21b847bc85749087a103cbb2a9f49a008e126722"
STALE_V4_REPO = "70fab66425a142307eab738783e32fbb93ec8fb6859c438007c7c6cd94165f77"
REF_FILES = PACK / "v5_reference" / "files" / "services" / "win5-ai"
...
def write_good_db(path: Path, index_sql: str | None = None, versions=OWNER_MIGRATIONS) -> None:
...
def copy_v5_tree(win5: Path, extra_present: str | None = None) -> None:
...
def fake_proc(_pid: int) -> dict[str, str | None]:
    return {
        ...
        "EXPECT_AI_ALLOW_MIGRATION_019": None,
...
def run_compare(
    ...
    *,
    ...
    if prepare_tree and (extra_present or not (win5 / "app" / "main.py").is_file()):
        copy_v5_tree(win5, extra_present=extra_present)
...
class GeneratedHashTests(unittest.TestCase):
    ...
    def test_all_11_match_v6_actual_files_and_file_sha256(self) -> None:
    ...
    def test_v6_db_and_repository_are_not_stale_v4_hashes(self) -> None:
        self.assertEqual(lc.DEPLOY["app/data/db.py"], V6_DB)
        self.assertEqual(lc.DEPLOY["app/data/repository/__init__.py"], V6_REPO)
        self.assertNotEqual(lc.DEPLOY["app/data/db.py"], STALE_V4_DB)
        self.assertNotEqual(lc.DEPLOY["app/data/repository/__init__.py"], STALE_V4_REPO)

    def test_snapshot_hash_is_correct_64_not_malformed_59(self) -> None:
        digest = lc.DEPLOY["app/predictions/snapshot.py"]
        self.assertEqual(digest, V6_SNAPSHOT)
    ...
    def test_correct_snapshot_py_is_match_candidate(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("LIVE app/predictions/snapshot.py %s MATCH_CANDIDATE" % V6_SNAPSHOT, stdout)
...
class SchemaCanonTests(unittest.TestCase):
    def test_happy_path_records_23_migrations_12_cols_and_019_absent(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            win5 = Path(td) / "services" / "win5-ai"
            rc, stdout = run_compare(win5)
            self.assertIn("SCHEMA_MIGRATIONS_COUNT=23", stdout)
            self.assertIn("PREDICTIONS_COLUMN_COUNT=12", stdout)
            self.assertIn("SCHEMA_HAS_019_PERSIST=NO", stdout)
            self.assertIn("SCHEMA_HAS_022=YES", stdout)
            self.assertIn("INDEX_CONTRACT=OK", stdout)
            self.assertNotIn("STOP_REASON=schema_019_persist_present", stdout)
            self.assertEqual(rc, 0)

    def test_019_persist_row_is_stop(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            win5 = Path(td) / "services" / "win5-ai"
            copy_v5_tree(win5)
            write_good_db(
                win5 / "var" / "expect_ai.db",
                versions=OWNER_MIGRATIONS + ("019_prediction_run_idempotency",),
            )
            fixture = systemd_fixture(win5)
            rc, stdout = run_compare(win5, fixture=fixture, prepare_tree=False)
            self.assertIn("SCHEMA_HAS_019_PERSIST=YES", stdout)
            self.assertIn("STOP_REASON=schema_019_persist_present", stdout)
...
class SystemdEvidenceGapTests(unittest.TestCase):
    ...
    def test_activestate_not_active_is_stop(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            win5 = Path(td) / "services" / "win5-ai"
            copy_v5_tree(win5)
            fixture = systemd_fixture(win5, ActiveState="inactive", MainPID="0")
            rc, stdout = run_compare(win5, fixture=fixture, prepare_tree=False)
            self.assertIn("SYSTEMD_ACTIVE=inactive", stdout)
            self.assertIn("STOP_REASON=systemd_unit_not_active", stdout)
            self.assertIn("COMPARE_RESULT=STOP", stdout)
            self.assertNotEqual(rc, 0)
    ...
    def test_loadstate_empty_is_stop(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            win5 = Path(td) / "services" / "win5-ai"
            copy_v5_tree(win5)
            fixture = systemd_fixture(win5, LoadState="", ActiveState="inactive", MainPID="0")
            rc, stdout = run_compare(win5, fixture=fixture, prepare_tree=False)
            self.assertIn("SYSTEMD_LOADSTATE=ABSENT", stdout)
            self.assertIn("STOP_REASON=systemd_unit_not_loaded", stdout)
            self.assertIn("COMPARE_RESULT=STOP", stdout)
            self.assertNotEqual(rc, 0)

    def test_loadstate_missing_is_stop(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            win5 = Path(td) / "services" / "win5-ai"
            copy_v5_tree(win5)
            fixture = systemd_fixture(win5, ActiveState="inactive", MainPID="0")
            del fixture["LoadState"]
            rc, stdout = run_compare(win5, fixture=fixture, prepare_tree=False)
            self.assertIn("SYSTEMD_LOADSTATE=ABSENT", stdout)
            self.assertIn("STOP_REASON=systemd_unit_not_loaded", stdout)
            self.assertIn("COMPARE_RESULT=STOP", stdout)
    ...
    def test_loadstate_loaded_and_active_keeps_happy_path(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            win5 = Path(td) / "services" / "win5-ai"
            rc, stdout = run_compare(win5)
            self.assertIn("SYSTEMD_LOADSTATE=loaded", stdout)
            self.assertIn("SYSTEMD_ACTIVE=active", stdout)
            self.assertNotIn("STOP_REASON=systemd_unit_not_loaded", stdout)
            self.assertNotIn("STOP_REASON=systemd_unit_not_active", stdout)
            self.assertIn("COMPARE_RESULT=GO", stdout)
            self.assertEqual(rc, 0)

    def test_019_flag_one_is_hard_stop(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            win5 = Path(td) / "services" / "win5-ai"
            copy_v5_tree(win5)

            def one_019(_pid: int) -> dict[str, str | None]:
                return {
                    "PREDICTION_RUNS_ENABLED": "0",
                    "EXPECT_AI_ALLOW_MIGRATION_022": None,
                    "EXPECT_AI_ALLOW_MIGRATION_019": "1",
                    "EXPECT_AI_DB_PATH": None,
                }

            rc, stdout = run_compare(win5, proc=one_019, prepare_tree=False)
            self.assertIn("EXPECT_AI_ALLOW_MIGRATION_019=ONE", stdout)
            self.assertIn("HARD_STOP_BEFORE_ANY_LATER_DEPLOY=YES", stdout)
            self.assertIn("STOP_REASON=EXPECT_AI_ALLOW_MIGRATION_019_is_ONE", stdout)
            self.assertIn("COMPARE_RESULT=STOP", stdout)
            self.assertNotEqual(rc, 0)


class GitReadonlyLockTests(unittest.TestCase):
    def test_all_git_invocations_use_no_optional_locks(self) -> None:
        sources = {
        ...
        git_sh_call = re.compile(r'sh\(\s*"git"\s*,\s*"([^"]+)"')
        writable_status = re.compile(
            r'sh\(\s*"git"\s*,\s*"(?!-?-no-optional-locks)[^"]*"[\s\S]{0,80}?"status"'
        )
        shell_git_status = re.compile(r"(?m)^\s*git\s+(?!-?-no-optional-locks).*\bstatus\b")
        for name, text in sources.items():
            calls = git_sh_call.findall(text)
            self.assertTrue(calls, msg="%s has no git sh() calls" % name)
            for first in calls:
                self.assertEqual(first, "--no-optional-locks", msg="%s first git arg %s" % (name, first))
            self.assertIsNone(writable_status.search(text), msg=name)
            self.assertIsNone(shell_git_status.search(text), msg=name)
            self.assertIn("def git_cmd(", text)
            self.assertIn('sh("git", "--no-optional-locks"', text)
            self.assertNotIn("GIT_OPTIONAL_LOCKS", text)
            self.assertNotIn("git config", text)
            self.assertIsNone(re.search(r"(?m)^\s*export\s+GIT_", text), msg=name)
...
class ExecStartAllowlistTests(unittest.TestCase):
    ...
    def test_happy_path_shows_executable_and_module(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("EXPECT_AI_ALLOW_MIGRATION_022=UNSET", stdout)
            self.assertIn("EXPECT_AI_ALLOW_MIGRATION_019=UNSET", stdout)
            self.assertEqual(rc, 0)
...
class ReadonlyStaticTests(unittest.TestCase):
    ...
    def test_canonical_equals_presented(self) -> None:
        ...
        self.assertIn(V6_SNAPSHOT.encode("ascii"), canon)
        self.assertIn(V6_DB.encode("ascii"), canon)
        self.assertNotIn(STALE_V4_DB.encode("ascii"), canon)
