...
FRESH_BACKUP_SHA = "32dfe70339a7237442d08a032d019ec03abbb8b85d0c90fa39d385e54da07e6a"
FRESH_BACKUP_LOG = "f3525b6fea85526083224b38a7d48271ae6a356c3af4bf67908efa07c852495d"
FINAL_V1_ZIP = "4f80248bc8d7520db66e12089be578ee9606e4a6d7bd9d22271d33d39bc32916"
...
def main() -> int:
    ...
    expect("APPLY_BACKUP_CANON=PINNED_FRESH_PRODUCTION_BACKUP" in flags, "fresh_backup_pinned", failures)
    expect(FRESH_BACKUP_SHA in flags and FRESH_BACKUP_SHA in stdin and FRESH_BACKUP_SHA in ps, "fresh_backup_sha_everywhere", failures)
    expect(FRESH_BACKUP_LOG in flags and FRESH_BACKUP_LOG in stdin, "fresh_backup_log_sha", failures)
    expect("20260912T013332Z" in flags and "20260912T013332Z" in stdin and "20260912T013332Z" in ps, "fresh_backup_path_everywhere", failures)
    expect("66305:287343" in flags and "66305:287343" in stdin, "fresh_backup_identity", failures)
    expect("APPLY_BACKUP_SIZE=107413504" in flags, "fresh_backup_size_flag", failures)
    expect("Test-ApplyBackupCanonReady" in ps, "ps1_canon_fn", failures)
    expect(ps.find("Test-ApplyBackupCanonReady") < ps.find("Invoke-TimedProcess -FileName 'ssh'"), "ps1_canon_before_ssh", failures)
    expect("APPLY_BACKUP_CANON_UNSET" in ps, "ps1_canon_unset_token", failures)
    expect("STALE_BACKUP_NOT_APPLY_CANON" in ps, "ps1_stale_token", failures)
    expect(V5_APPLY_ZIP in flags, "v5_apply_zip_recorded", failures)
    expect("V5_APPLY_ZIP_OVERWRITE=NO" in flags, "v5_apply_kept", failures)
    expect("FRESH_PRODUCTION_BACKUP_COMPLETE=YES" in flags, "flags_fresh_backup_complete", failures)
    expect("BACKUP_SIZE_MISMATCH" in stdin, "backup_size_halt", failures)
    expect("BACKUP_PATH_MISMATCH" in stdin, "backup_path_halt", failures)
    ...
    expect("V5_APPLY_ZIP_OVERWRITE=NO" in ps, "ps1_keeps_v5_zip", failures)
    ...
    expect(file_sha(STDIN) == EXPECTED_STDIN_SHA, "stdin_sha_byte_identical", failures)
    expect(EXPECTED_STDIN_SHA in ps, "ps1_pins_expected_stdin_sha", failures)
    expect("Test-StdinPayloadShaReady" in ps, "ps1_stdin_sha_fn", failures)
    expect(ps.find("Test-StdinPayloadShaReady") < ps.find("Invoke-TimedProcess -FileName 'ssh'"), "ps1_stdin_sha_before_ssh", failures)
    expect(ps.find("STDIN_PAYLOAD_SHA256_MATCH=NO") < ps.find("Invoke-TimedProcess -FileName 'ssh'"), "ps1_match_no_before_ssh", failures)
    expect(ps.find("STDIN_PAYLOAD_SHA256_MATCH=YES") < ps.find("Invoke-TimedProcess -FileName 'ssh'"), "ps1_match_yes_before_ssh", failures)
    expect("HALT_REASON=STDIN_PAYLOAD_SHA256_MISMATCH" in ps, "ps1_stdin_mismatch_halt", failures)
    expect("FINAL_022_APPLY_PACK_REVIEW=CHANGES_REQUIRED" in flags, "flags_final_review_changes", failures)
    expect("FINAL_022_APPLY_PACK_REVIEW_RESULT=CHANGES_REQUIRED" in flags, "flags_final_review_result", failures)
    expect("FINAL_022_APPLY_PACK_REVIEWED=YES" in flags, "flags_final_reviewed", failures)
    expect("FINAL_022_APPLY_PACK_EXECUTION_ALLOWED=NO" in flags, "flags_final_exec_no", failures)
    expect("FINAL_PACK_ZIP_OVERWRITE=NO" in flags, "flags_final_zip_kept", failures)
    expect(FINAL_V1_ZIP in flags, "final_v1_zip_recorded", failures)
    expect("PACK_VERSION=final_v2" in flags, "flags_pack_version_v2", failures)
    expect("STDIN_PAYLOAD_SHA256_ENFORCED=YES" in flags, "flags_stdin_sha_enforced", failures)
    expect("LOCAL_VERIFY=PENDING_WINDOWS_PS51_PARSEFILE" in flags, "flags_local_verify_pending", failures)
    expect("OFFICIAL_ALL_PASS=NO" in flags, "flags_no_official_all_pass", failures)
    local_verify = (ROOT / "05_local_verify.txt").read_text(encoding="utf-8")
    expect("LOCAL_VERIFY=PENDING_WINDOWS_PS51_PARSEFILE" in local_verify, "local_verify_pending", failures)
    expect("LOCAL_VERIFY=ALL_PASS" not in local_verify, "local_verify_not_all_pass", failures)
    expect("OFFICIAL_ALL_PASS=NO" in local_verify, "local_verify_official_all_pass_no", failures)
    run_src = (ROOT / "run_tests.py").read_text(encoding="utf-8")
    expect('print("ALL_PASS" if fail == 0 else "FAIL_COUNT=%d" % fail)' not in run_src, "run_tests_no_official_all_pass", failures)
    expect("LOCAL_TESTS_PASS_PENDING_WINDOWS_PS51_PARSEFILE" in run_src, "run_tests_pending_token", failures)
    expect("CORRECT_PAYLOAD_FULL_PS1_WITH_APPROVAL=NOT_RUN" in run_src, "run_tests_does_not_ssh_correct_payload", failures)
    expect("OWNER_PRODUCTION_022_APPLY_APPROVED\"] = \"1\"" in run_src, "run_tests_mismatch_uses_approval_one", failures)
