...
RACE_INDEX_COLS = ("race_id", "created_at")
OWNER_MIGRATIONS = (
    "001_init",
    "002_race_identity",
    "003_supply_platform",
...
def _http_fixture(url: str, endpoint_class: str, limit: int) -> tuple[int, bytes, str, str, str]:
    ...
    if path == "/health":
        ...
        if fail == "oversized_detail":
            raw = b"x" * (limit + 1)
            status = 200
        elif fail == "detail_bad_json":
            raw = b"{not-json"
            status = 200
        else:
...
def inspect_partial_index(conn: sqlite3.Connection) -> str:
...
def verify_pre_schema(src: Path) -> None:
    conn = open_ro(src)
    try:
        versions = [str(r[0]) for r in conn.execute("SELECT version FROM schema_migrations")]
        cols = [str(r[1]) for r in conn.execute("PRAGMA table_info(predictions)")]
        have = set(versions)
        owner = set(OWNER_MIGRATIONS)
        missing = sorted(owner - have)
        extra = sorted(have - owner)
        dup_count = len(versions) - len(have)
        new_present = [c for c in NEW_COLS if c in cols]
        emit("SCHEMA_MIGRATIONS_COUNT", len(versions))
        emit("SCHEMA_MIGRATIONS_UNIQUE_COUNT", len(have))
        emit("SCHEMA_MIGRATIONS_MISSING_COUNT", len(missing))
        emit("SCHEMA_MIGRATIONS_EXTRA_COUNT", len(extra))
        emit("SCHEMA_MIGRATIONS_DUPLICATE_COUNT", dup_count)
        emit("HAS_PERSIST_022", PERSIST_022 in have)
        emit("HAS_PERSIST_019", PERSIST_019 in have)
        emit("PREDICTIONS_COLUMN_COUNT", len(cols))
        emit("NEW_PERSIST_COLUMNS_PRESENT", len(new_present))
        emit("PARTIAL_UNIQUE_INDEX_STATE", inspect_partial_index(conn))
        emit("EXISTING_RACE_INDEX", inspect_race_index(conn))
        if dup_count != 0:
            raise Halt("MIGRATION_DUPLICATE")
        if PERSIST_022 in have:
            raise Halt("HAS_PERSIST_022")
        if PERSIST_019 in have:
            raise Halt("HAS_PERSIST_019")
        if len(versions) != 22 or have != owner or missing or extra:
            raise Halt("MIGRATION_SET_MISMATCH")
        if cols != list(OWNER_PRED_COLUMNS):
            raise Halt("PREDICTIONS_COLUMNS_MISMATCH")
        if new_present:
            raise Halt("NEW_PERSIST_COLUMNS_PRESENT")
        if inspect_partial_index(conn) != "missing":
            raise Halt("PARTIAL_UNIQUE_INDEX_PRESENT")
        if inspect_race_index(conn) != "ok":
            raise Halt("EXISTING_INDEX_MISSING_OR_MISMATCH")
        emit("SOURCE_SCHEMA_GATE", "PASS")
        emit("HTTP_GET_ALLOWED", "YES")
    finally:
        conn.close()
...
def fail_closed(code: str) -> int:
    ...
    emit("DB_CHANGED", "NO")
    emit("CACHE_WRITE", "NOT_CHECKED")
    emit("FILE_WRITE", "NOT_CHECKED")
    emit("CACHE_OR_FILE_WRITE_VERIFIED", "NO")
...
def run() -> int:
    ...
    emit("V1_MEASURE_ZIP_OVERWRITE", "NO")
    emit("CACHE_WRITE", "NOT_CHECKED")
    emit("FILE_WRITE", "NOT_CHECKED")
    emit("CACHE_OR_FILE_WRITE_VERIFIED", "NO")
    emit("HTTP_GET_ALLOWED", "NO")
    emit("SOURCE_SCHEMA_GATE", "PENDING")
    ...
    health_status, health_payload, health_over = measure_http(localhost_ai("/health"), endpoint_class=CLASS_INTERNAL_HEALTH, headers={"Accept": "application/json"})
    emit("INTERNAL_HEALTH_OVERSIZED", health_over)
    if health_over:
        raise Halt("HTTP_BODY_OVERSIZED")
    if health_status != 200:
        raise Halt("HEALTH_FAIL")
    if health_payload is None:
        raise Halt("HEALTH_JSON_PARSE_FAIL")
    ...
    emit("INTERNAL_LIST_OVERSIZED", list_over)
    if list_over:
        after = pred_row_count(src)
        emit("PRED_ROW_AFTER", after)
        emit("PRED_ROW_DELTA", after - before)
        emit("DETAIL_GET", "SKIPPED_LIST_OVERSIZED")
        emit("INTERNAL_DETAIL_HTTP_STATUS", "SKIPPED")
        emit("INTERNAL_DETAIL_OVERSIZED", "SKIPPED")
        emit("INTERNAL_DETAIL_JSON_PARSE", "SKIPPED")
        assert_no_write_calls()
        if after != before:
    ...
    if list_payload is None:
        raise Halt("GET_JSON_PARSE_FAIL")
    ...
    if not rid:
        ...
        detail_status, detail_payload, detail_over = measure_http(
            localhost_ai("/v1/predictions/%s" % rid),
            endpoint_class=CLASS_INTERNAL_DETAIL,
            headers=ai_headers(),
        )
        emit("DETAIL_GET", "ATTEMPTED")
        emit("DETAIL_GET_AUTHORIZATION_SENT", "NO")
        emit("INTERNAL_DETAIL_HTTP_STATUS", detail_status)
        emit("INTERNAL_DETAIL_OVERSIZED", detail_over)
        emit("INTERNAL_DETAIL_JSON_PARSE", (not detail_over) and detail_payload is not None)
        if detail_over:
            after = pred_row_count(src)
            emit("PRED_ROW_AFTER", after)
            emit("PRED_ROW_DELTA", after - before)
        ...
        if detail_payload is None:
            raise Halt("DETAIL_JSON_PARSE_FAIL")
    after = pred_row_count(src)
    emit("PRED_ROW_AFTER", after)
    emit("PRED_ROW_DELTA", after - before)
    if after != before:
        raise Halt("GET_PRED_ROW_DELTA_NONEZERO")
    assert_no_write_calls()
    return finish_success(partial=STATE.body_oversized or detail_over)
