...
def _http_fixture(method: str, url: str) -> tuple[int, object]:
    parsed = urlparse(url)
    path = redact_path(parsed.path or "/")
    host = parsed.netloc
    public = host in ("expect-keiba.com", "keiba-single-ai.pages.dev")
    fail = test_fail_mode()
...
def http_get(url: str, *, headers: dict[str, str] | None = None, allow_auth_header: bool = False) -> tuple[int, object]:
    ...
    if test_mode() and (os.environ.get("OWNER_APPLY_TEST_HTTP") or "").strip() == "1":
        return _http_fixture(method, url)
...
def run_public_smoke() -> None:
    ...
    for url in urls:
        parsed = urlparse(url)
        path = parsed.path or "/"
        status, payload = http_get(url, headers={"Accept": "text/html,application/json"}, allow_auth_header=False)
        emit("PUBLIC_SMOKE_STATUS", "%s://%s%s:%s" % (parsed.scheme, parsed.netloc, path, status))
        checked += 1
        if path == "/api/predictions":
            ok = status == 200 or ops_closed(status, payload)
            emit("PUBLIC_PREDICTIONS_OPS_CLOSED", ops_closed(status, payload))
            if not ok:
                raise Halt("PUBLIC_SMOKE_FAIL")
            continue
        if path == "/api/health":
