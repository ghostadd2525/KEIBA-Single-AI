...
$OwnerPs1 = Join-Path $Here 'OWNER_MEASURE.ps1'
$fail = 0
...
$pred = { param($a) return ($a -is [System.Management.Automation.Language.FunctionDefinitionAst]) }
$fns = $ast.FindAll($pred, $true)
$needed = @('Stop-ProcessTree', 'Get-TaskText', 'Get-WrapperAuditStatus', 'Get-RemoteMeasureState', 'Invoke-TimedProcess', 'Test-OwnerApprovalReady', 'Get-Sha256Hex')
foreach ($name in $needed) {
    $hit = $false
    foreach ($fn in $fns) { if ($fn.Name -eq $name) { $hit = $true } }
    Expect $hit ('ast_has_' + $name)
    if ($hit) {
        foreach ($fn in $fns) { if ($fn.Name -eq $name) { Invoke-Expression $fn.Extent.Text } }
    }
}

$invokeFn = $null
foreach ($fn in $fns) { if ($fn.Name -eq 'Invoke-TimedProcess') { $invokeFn = $fn } }
if ($null -ne $invokeFn) {
...
Expect ($allText.Contains('$env:OWNER_PRODUCTION_022_MEASURE_APPROVED =') -eq $false) 'wrapper_does_not_set_approval'
...
Expect ($allText.Contains('owner_measure_stdin.py') -eq $true) 'reads_pregenerated_stdin'
...
$td = Join-Path ([System.IO.Path]::GetTempPath()) ('measpaths-' + [guid]::NewGuid().ToString('N'))
...
$sf = Invoke-TimedProcess -FileName '/no/such/measure_ssh_binary' -Arguments '' -TimeoutMs 5000 -StdinText $null
