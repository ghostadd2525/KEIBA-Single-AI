...
IMMUTABLE_ZIPS = (
    ...
    "production_disabled_post_hunk_only_deploy_review_v2_20260913.zip",
)
...
class HunkOnlyReviewTests(unittest.TestCase):
    ...
    def test_flags_forbid_deploy_and_keep_disabled(self) -> None:
        ...
        self.assertEqual(flags["LIVE_OPS_PRESENT_IN_THIS_BUILD"], "YES")
    ...
    def test_readme_and_donot_forbid_execution(self) -> None:
        ...
        for text in (readme, donot):
            ...
            self.assertIn("INDEPENDENT_REVIEW_OF_HUNK_ONLY_V3_BUNDLE", text)
    ...
    def test_no_execution_scripts(self) -> None:
        ...
        for path in PACK.rglob("*"):
            ...
            if path.suffix in {".py", ".txt"}:
                if path.name == "test_hunk_only_v3_review.py":
                    continue
    ...
    def test_hunk_v1_zip_not_in_pack(self) -> None:
        ...
        self.assertFalse((PACK / "production_disabled_post_hunk_only_deploy_review_v2_20260913.zip").exists())
        flags = parse_flags(PACK / "FLAGS.txt")
        ...
        self.assertEqual(flags["HUNK_ONLY_V2_ZIP_SHA256"], "9de787a0089ca2f6d3f5388f788a2f6246469dcc35ac3f110bcd1758952c3518")
        self.assertEqual(flags["HUNK_ONLY_V2_NOT_OVERWRITTEN"], "YES")

    def test_v3_readme_requires_overlay_suite(self) -> None:
    ...
    def test_live_ops_present_and_overlay_fail_closed_path_remains(self) -> None:
        ...
        self.assertEqual(cap.stat().st_size, 15241)
        self.assertEqual(store.stat().st_size, 22775)
        self.assertEqual(sha256_file(cap), "c351bd9753d20994156e00141d564a55a42891983ecab5b2c5c55a32d9908f1e")
        self.assertEqual(sha256_file(store), "40352ff56b3267533267d5de9c893f3bda8963d5fe121474f7785b0152594a12")
        self.assertFalse((PACK / "live_canon/app/ops/LIVE_BYTES_REQUIRED.txt").exists())
        src = (PACK / "run_tests.py").read_text(encoding="utf-8")
        ...
        self.assertNotIn("CAPTURE_TOOL_FIXTURE", cap.read_text(encoding="utf-8"))
        self.assertNotIn("CAPTURE_TOOL_FIXTURE", store.read_text(encoding="utf-8"))
    ...
    def test_owner_intake_doc_lists_return_zip_shas(self) -> None:
        ...
        self.assertIn("LIVE_OPS_PRESENT_IN_THIS_BUILD=YES", text)
        self.assertIn("DEPLOY_CANDIDATE", (PACK / "ingest_owner_ops_canon.py").read_text(encoding="utf-8"))

    def test_ingest_reads_windows_backslash_zip_members(self) -> None:
        import ingest_owner_ops_canon as ingest

        payload = (PACK / "live_canon/app/ops/prediction_capacity.py").read_bytes()
        zip_path = _windows_zip_path(payload)
        data = ingest.read_named_from_zip(zip_path, "prediction_capacity.py")
        self.assertEqual(data, payload)
        self.assertEqual(ingest.zip_member_posix(
            r"prediction_capacity_capture_windows_20260913\extracted\app\ops\prediction_capacity.py"
        ), "prediction_capacity_capture_windows_20260913/extracted/app/ops/prediction_capacity.py")


def _windows_zip_path(payload: bytes):
    import tempfile
    import zipfile
    from pathlib import Path

    tmp = Path(tempfile.mkdtemp(prefix="winzip-")) / "owner_return.zip"
