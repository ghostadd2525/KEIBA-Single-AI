...
class Handler(BaseHTTPRequestHandler):
    ...
    def _do_GET_impl(self) -> None:
        ...
        # Optional integration facades (soft-import: missing packages must not break core routes)
        try:
        ...
        if m:
            race_id = m.group(1)
            bundle, meta = prediction_capacity.get_with_meta(race_id)
            if not bundle:
        ...
        if path == "/v1/challenge/active":
            ...
            payload = get_lifecycle_service().list_active(ctx.user_id)
            self._send(200, ok(payload, {"service": "ChallengeLifecycle"}))
            return
        ...
        if path == "/v1/notifications":
            ...
            payload = get_lifecycle_service().list_notifications(
                ctx.user_id, limit=max(1, min(limit, 200))
            )
            self._send(200, ok(payload, {"service": "ChallengeLifecycle"}))
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        # Optional integration facades (soft-import)
        try:
            ...
            SingleServiceConfig = None  # type: ignore
            si_try_post = None
        try:
            ...
            SiteIntegrationConfig = None  # type: ignore
            site_try_post = None
        try:
        ...
        if path.startswith("/v1/ui/") and ui_try_post is not None:
            ui_post = ui_try_post(
                path,
        ...
        if path.startswith("/v1/site/") and site_try_post is not None and SiteIntegrationConfig is not None:
        ...
        if path.startswith("/v1/single/") and si_try_post is not None and SingleServiceConfig is not None:
        ...
        m_life_viewed = re.fullmatch(
            r"/v1/challenge/lifecycle/([^/]+)/viewed", path
        )
        if m_life_viewed:
            ctx, denied = self._user_auth()
            if denied:
                ...
                return
            assert ctx is not None
            race_id = m_life_viewed.group(1)
            from .challenge.lifecycle import get_lifecycle_service

            try:
                reveal = True
                if isinstance(body, dict) and "reveal_completed" in body:
                    reveal = bool(body.get("reveal_completed"))
                payload = get_lifecycle_service().mark_viewed(
                    ctx.user_id, race_id, reveal_completed=reveal
                )
                self._send(200, ok(payload, {"service": "ChallengeLifecycle"}))
        ...
        m_notif_read = re.fullmatch(r"/v1/notifications/([^/]+)/read", path)
        if m_notif_read:
            ctx, denied = self._user_auth()
            if denied:
                self._send(*denied)
            ...
            try:
                nid = int(m_notif_read.group(1))
                payload = get_lifecycle_service().mark_notification_read(
                    ctx.user_id, nid
                )
                self._send(200, ok(payload, {"service": "ChallengeLifecycle"}))
