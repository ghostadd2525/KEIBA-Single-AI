...
def zip_member_posix(name: str) -> str:
    """Owner Windows return ZIPs store members with backslash separators."""
    return name.replace("\\", "/").rstrip("/")


def is_zip_dir_member(name: str) -> bool:
...
def read_named_from_zip(zip_path: Path, filename: str) -> bytes:
    with zipfile.ZipFile(zip_path) as zf:
        ...
        for n in zf.namelist():
            if is_zip_dir_member(n):
                continue
            posix = zip_member_posix(n)
