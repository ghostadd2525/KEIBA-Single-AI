# PowerShell 5.1. Owner-only Production read-only restart-fail probe.
# Owner runs this file only. Cursor does not run this on Production.
# Sends pregenerated owner_probe_stdin.py byte-for-byte on SSH stdin.
# Does not concatenate Python files at runtime. No SCP. No remote mkdir.
# Wrapper never sets OWNER_READONLY_STEP1_PROBE_APPROVED.
# Wrapper never sets or forwards OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED.
# Does not run OWNER_DEPLOY.ps1. Does not restart or reload.
...
$StdinPy = Join-Path $Here 'owner_probe_stdin.py'
$SumsFile = Join-Path $PackRoot 'SHA256SUMS.txt'
$FlagsFile = Join-Path $PackRoot 'FLAGS.txt'
$Downloads = Join-Path $env:USERPROFILE 'Downloads'
$Stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$OutLog = Join-Path $Downloads ('production_step1_restart_fail_readonly_probe_20260913_output_' + $Stamp + '.txt')
$TimeoutMs = 300000
$script:KillDrainWaitMs = 8000
$script:DrainWaitMs = 20000
# Remote hard deadline is 180s. Wrapper must exceed remote + drain + kill-drain + 40s buffer.

if (Test-Path -LiteralPath $OutLog) {
    Write-Output 'OWNER_PROBE_STATUS=FAIL'
...
function Get-RemoteProbeState {
    ...
    if (-not [string]::IsNullOrEmpty($StdOut)) {
        ...
        foreach ($ln in $lines) {
            if ($ln.StartsWith('PROBE_EXECUTED=')) {
                $executed = $ln.Substring(15)
            }
            if ($ln.StartsWith('PROBE_PHASE=')) {
                $phase = $ln.Substring(12)
            }
            if ($ln.StartsWith('SYSTEMCTL_MUTATE_ISSUED=')) {
                $mutated = $ln.Substring(24)
            }
        }
    }
    return @{
        ProbeExecuted = $executed
        ProbePhase = $phase
        MutateIssued = $mutated
    }
}
...
function Test-OwnerApprovalReady {
    ...
    if ($Approved -ne '1') {
        return @{
            Ready = $false
            Reason = 'OWNER_READONLY_STEP1_PROBE_APPROVED_UNSET'
        }
    }
    return @{
        Ready = $true
...
Set-Content -LiteralPath $OutLog -Value 'PRODUCTION_STEP1_RESTART_FAIL_READONLY_PROBE_20260913' -Encoding UTF8
Write-Log ('STARTED=' + (Get-Date -Format 'yyyy-MM-ddTHH:mm:ss'))
Write-Log 'WRAPPER_VERSION=owner_readonly_step1_probe_20260913'
...
Write-Log 'REMOTE_HARD_DEADLINE_S=180'
Write-Log 'WRAPPER_TIMEOUT_MS=300000'
Write-Log 'DRAIN_WAIT_MS=20000'
Write-Log 'KILL_DRAIN_WAIT_MS=8000'
Write-Log 'SAFETY_BUFFER_MS=40000'
Write-Log 'PACK_KIND=OWNER_READONLY_RESTART_FAIL_STATE_PROBE'
...
Write-Log 'SUDO_ALLOWED=SUDO_N_L_PERMISSION_LIST_ONLY'
Write-Log 'SYSTEMCTL_RESTART=NO'
Write-Log 'SYSTEMCTL_RELOAD=NO'
...
Write-Log 'WRAPPER_SETS_OWNER_READONLY_STEP1_PROBE_APPROVED=NO'
...
Write-Log 'OWNER_DEPLOY_PS1_RERUN=NO'
...
if (-not $shaGate.Ok) {
    ...
    Write-Log 'OWNER_PROBE_STATUS=FAIL'
    Write-Log 'PROBE_EXECUTED=NO'
...
$approvedText = [string]$env:OWNER_READONLY_STEP1_PROBE_APPROVED
...
Write-Log 'STDIN_SOURCE=owner_probe_stdin.py'
# Forward only the Owner-set probe approval value 1. Wrapper never assigns the Windows env.
# Do not forward OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED.
# Do not forward EXPECT_AI_ALLOW_MIGRATION_022/019 or PREDICTION_RUNS_ENABLED.
$remoteCmd = 'OWNER_READONLY_STEP1_PROBE_APPROVED=1 PYTHONDONTWRITEBYTECODE=1 python3 -'
...
$remoteState = Get-RemoteProbeState -StdOut ([string]$remote.StdOut)
Write-Log ('REMOTE_PROBE_EXECUTED=' + [string]$remoteState.ProbeExecuted)
Write-Log ('REMOTE_PROBE_PHASE=' + [string]$remoteState.ProbePhase)
Write-Log ('REMOTE_SYSTEMCTL_MUTATE_ISSUED=' + [string]$remoteState.MutateIssued)
...
if ($remote.TimedOut) {
    ...
    $committedKnown = (($remoteState.ProbeExecuted -eq 'YES') -and ($remoteState.ProbePhase -eq 'COMPLETE'))
