...
STDIN = PS / "owner_probe_stdin.py"
PS1 = PS / "OWNER_PROBE.ps1"
...
def main() -> int:
    ...
    saved = Path(tempfile.mkdtemp(prefix="probe-payload-")) / "sent_stdin_payload.py"
    ...
    expect("owner_probe_stdin.py" in ps, "ps1_reads_stdin_file", failures)
    ...
    expect("OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED=1" not in ps, "ps1_does_not_forward_deploy", failures)
    ...
    expect("HALT_REASON=OWNER_READONLY_STEP1_PROBE_APPROVED_UNSET" in out, "python3_dash_unapproved_reason", failures)
    expect("PROBE_EXECUTED=NO" in out, "unapproved_not_executed", failures)
    expect("SYSTEMCTL_MUTATE_ISSUED=NO" in out, "unapproved_no_mutate", failures)
