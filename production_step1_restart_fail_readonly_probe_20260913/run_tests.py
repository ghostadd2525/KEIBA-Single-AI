...
def finalize_evidence_and_sums():
    ...
    dest = Path(tempfile.mkdtemp(prefix="probe1-sums-")) / "production_step1_restart_fail_readonly_probe_20260913"
...
def main():
    ...
    env.pop("OWNER_READONLY_STEP1_PROBE_APPROVED", None)
    ...
    r = subprocess.run([sys.executable, str(ROOT / "03_tests" / "test_probe_local.py")], capture_output=True, text=True, env=env)
    print(r.stdout)
    if r.stderr:
        print(r.stderr)
    expect(r.returncode == 0 and "ALL_PASS" in (r.stdout or ""), "probe_local")
    ...
    for name in (
        ...
        "wrapper_does_not_forward_deploy_approval",
    ):
        expect(("PASS " + name) in (paths.stdout or ""), "paths_" + name)

    tmp = Path(tempfile.mkdtemp(prefix="probe1-halt-"))
    downloads = tmp / "Downloads"
    downloads.mkdir()
    halt_env = env.copy()
    halt_env["USERPROFILE"] = str(tmp)
    halt_env.pop("OWNER_READONLY_STEP1_PROBE_APPROVED", None)
    ...
    expect("OWNER_READONLY_STEP1_PROBE_APPROVED_UNSET" in (halt.stdout or ""), "ps1_unapproved_reason")
    logs = list(downloads.glob("production_step1_restart_fail_readonly_probe_20260913_output_*.txt"))
    ...
    if logs:
        ...
        expect("OWNER_PRODUCTION_STEP1_DEPLOY_APPROVED=1" not in body, "ps1_log_no_deploy_approval_forward")
