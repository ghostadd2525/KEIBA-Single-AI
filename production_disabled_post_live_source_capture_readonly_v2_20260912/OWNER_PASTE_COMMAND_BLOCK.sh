...
echo "===== BEGIN OWNER_READ_ONLY_LIVE_SOURCE_CAPTURE_V2 ====="
echo "PACK=production_disabled_post_live_source_capture_readonly_v2_20260912"
...
def emit_judgment(captured: int) -> str:
    ...
    out("NEXT_STEP=INDEPENDENT_REVIEW_OF_READONLY_LIVE_SOURCE_CAPTURE_V2")
    return result


def main() -> int:
    reset_state()
    out("PACK=production_disabled_post_live_source_capture_readonly_v2_20260912")
    out("KIND=OWNER_READ_ONLY_LIVE_SOURCE_CAPTURE_V2")
...
rc=$?
echo "===== END OWNER_READ_ONLY_LIVE_SOURCE_CAPTURE_V2 ====="
exit "$rc"
