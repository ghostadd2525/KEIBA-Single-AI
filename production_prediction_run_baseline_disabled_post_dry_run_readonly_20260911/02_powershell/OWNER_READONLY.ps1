# PowerShell 5.1. Owner-only. READ-ONLY Production Step 2-4 baseline / disabled-POST DRY_RUN.
# Cursor does not run this. Do not run v1 smoke. Do not overwrite v1-v4 review ZIP.
# No Production writes. No SCP. No GitHub. No 019 apply. No POST.
# Remote Python may GET http://127.0.0.1:8000 only.
# Never POST /v1/prediction-runs. Never GET /v1/predictions/{race_id}.
...
$OutLog = Join-Path $Downloads ('production_prediction_run_baseline_disabled_post_dry_run_readonly_20260911_output_' + $Stamp + '.txt')
$TimeoutMs = 180000
...
Set-Content -LiteralPath $OutLog -Value 'PRODUCTION_PREDICTION_RUN_BASELINE_DISABLED_POST_DRY_RUN_READONLY_20260911' -Encoding UTF8
Write-Log ('STARTED=' + (Get-Date -Format 'yyyy-MM-ddTHH:mm:ss'))
Write-Log 'WRAPPER_VERSION=owner_readonly_baseline_disabled_post_dry_run_20260911'
